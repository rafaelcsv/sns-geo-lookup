---
title: "Tabela de pesquisa das áreas de abrangência das Unidades Locais de Saúde (versão de 2023)"
author:
...
# Ligação ACES para ULS

Incorporar novas ULS de 2024.

```{r linkage-uls}
# loads ULS linkage file
uls_reference <- import("inputs/reorganizacao-uls-2024.xlsx") |> clean_names() |> rename(uls_2024 = designacao, aces_2022 = componentes_curto) |> mutate(
  ...
  filter(!str_detect(aces_2022, "Centro de Medicina de Reabilitação ")) |>
  filter(!str_detect(aces_2022, "Hospital ")) |>
  filter(!str_detect(aces_2022, "Centro Hospitalar ")) |>
  filter(!str_detect(aces_2022, "Instituto de Oftalmologia ")) |>
  filter(!str_detect(aces_2022, "Laboratório de Saúde Pública ")) |>
  select(uls_2024, aces_2022)

# links ACES and ULS, the column order is defined
sns_geo_lookup <- full_join(uls_reference, sns_geo_lookup, by = "aces_2022")
  
  #select(freguesia_2013, dicofre_2013, municipio_2013, municipio_2013_cod, municipio_2002_cod, distrito_2013, distrito_2013_cod, nuts3_2013, nuts3_2013_cod, nuts3_2002_cod, nuts2_2013, nuts2_2013_cod, nuts1_2013, nuts1_2013_cod, pais, pais_cod, aces_2022, aces_2022_cod, ars_2022, ars_2022_cod, uls_2024)
```
...
```{r results}
# saves as .csv
write.csv(sns_geo_lookup, file = "outputs/snss-geo-lookup.csv", fileEncoding = "UTF-8", row.names = FALSE)
